...
var Charts = (function () {
  ...
  /* ---------- the book-rain box (bans over time) ---------- */

  /*
   * A single <canvas> 2.5D particle field: pick a year span with the two
   * sliders and one book falls for every ~12 gazetted publications in it
   * (hard cap 300 sprites). The RAF loop runs only while the section is on
   * screen AND books are still falling; under prefers-reduced-motion the
   * settled end-state renders immediately. The Year / Decade / Cumulative
   * chart views are the accessible equivalent and carry the real numbers.
   */
  var RAIN_PER_SPRITE = 12;
  var RAIN_CAP = 300;

  function bookRain(cfg) {
    var box = cfg.box;
    var fromEl = cfg.from, toEl = cfg.to, readout = cfg.readout;
    ...
    var sprites = [];      /* {x,y,vy,rot,vr,tx,ty,tr,ci,settled} */
    var spawnQueue = 0;
    var visible = false;
    var running = false;
    var W = 0, H = 0, DPR = 1;
    ...
    /* deterministic jitter so resize re-lays the same pile */
    function jit(i, salt) {
    ...
    function placeOf(i) {
      var cols = Math.max(4, Math.floor((W - 24) / (BW + 3)));
      var row = Math.floor(i / cols);
      var col = i % cols;
      /* offset alternate rows half a book, brick-like */
      var x = 12 + col * (BW + 3) + (row % 2 ? BW / 2 : 0) + (jit(i, 1) - 0.5) * 4;
      var y = H - 8 - BH - row * (BH + 1.6);
      return {
        x: Math.min(W - BW - 6, Math.max(6, x)),
        y: y,
        rot: (jit(i, 2) - 0.5) * 0.16
      };
    }
    ...
    function syncSize() {
      ...
      sprites.forEach(function (s, i) {
        var p = placeOf(i);
        s.tx = p.x; s.ty = p.y; s.tr = p.rot;
        if (s.settled) { s.x = p.x; s.y = p.y; s.rot = p.rot; }
      });
      paint();
    }

    function bookColor(ci, th) {
      return th.slots[ci % th.slots.length];
    }
    ...
    function paint() {
      ...
      ctx.beginPath();
      ctx.moveTo(6, H - 6.5);
      ctx.lineTo(W - 6, H - 6.5);
      ctx.stroke();
      sprites.forEach(function (s) {
        ctx.save();
        ctx.translate(s.x + BW / 2, s.y + BH / 2);
        ctx.rotate(s.rot);
        ctx.fillStyle = bookColor(s.ci, th);
        ctx.globalAlpha = 0.92;
        ctx.fillRect(-BW / 2, -BH / 2, BW, BH);
        /* pale spine stripe */
        ctx.globalAlpha = 0.5;
        ctx.fillStyle = th.surface;
        ctx.fillRect(-BW / 2 + 2, -BH / 2 + 1, 2, BH - 2);
        ctx.restore();
      });
    }
    ...
    function step() {
      ...
      if (spawnQueue > 0) {
        ...
        for (var b = 0; b < batch; b++) {
          ...
          var p = placeOf(i);
          sprites.push({
            x: p.x + (jit(i, 3) - 0.5) * 30,
            y: -14 - jit(i, 4) * H * 0.5,
            vy: 0.6 + jit(i, 5) * 2,
            rot: (jit(i, 6) - 0.5) * 1.4,
            vr: (jit(i, 7) - 0.5) * 0.06,
            tx: p.x, ty: p.y, tr: p.rot, ci: i % 6,
            settled: false
          });
        }
        spawnQueue -= batch;
      ...
      sprites.forEach(function (s) {
        if (s.settled) return;
        s.vy = Math.min(s.vy + 0.22, 9);
        s.y += s.vy;
        s.x += (s.tx - s.x) * 0.08;
        s.rot += s.vr;
        if (s.y >= s.ty) {
          s.y = s.ty; s.x = s.tx; s.rot = s.tr;
          s.settled = true;
        } else {
          active = true;
        }
    ...
    function wake() {
      if (!running && visible && !reduced.matches) {
        running = true;
        requestAnimationFrame(step);
      }
    }
    ...
    function settleAll() {
      ...
      sprites.forEach(function (s, i) {
        var p = placeOf(i);
        s.x = p.x; s.y = p.y; s.rot = p.rot; s.settled = true;
      });
      paint();
    }
    ...
    function update() {
      ...
      readout.textContent = span.from + "–" + span.to + " · " + fmt(span.count) +
        " publications banned";
      var want = targetSprites(span.count);
      var have = sprites.length + spawnQueue;
      if (want < sprites.length) {
        sprites.length = want;
        spawnQueue = 0;
        paint();
      } else if (want > have) {
        spawnQueue += want - have;
      }
      if (reduced.matches) {
        while (spawnQueue > 0) {
          var i = sprites.length;
          var p = placeOf(i);
          sprites.push({ x: p.x, y: p.y, rot: p.rot, tx: p.x, ty: p.y, tr: p.rot, vy: 0, vr: 0, ci: i % 6, settled: true });
          spawnQueue--;
        }
        paint();
      } else {
    ...
    drawFns.push(paint); /* repaint with the new palette on theme change */
    ...
    if (reduced.matches) {
      ...
      readout.textContent = span.from + "–" + span.to + " · " + fmt(span.count) + " publications banned";
      paint();
    } else {
      update();
    }
  ...
  function init() {
    ...
    /* --- bans over time: the book-rain box + Year / Decade / Cumulative --- */
    ...
    if (rainBox) {
      ...
      bookRain({
        box: rainBox,
        from: document.getElementById("rain-from"),
        to: document.getElementById("rain-to"),
        readout: document.getElementById("rain-readout"),
        buttons: Array.prototype.slice.call(document.querySelectorAll(".rain-views button")),
        views: {
